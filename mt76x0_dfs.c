...
static const struct mt76x0_radar_specs etsi_radar_specs[] = {
	/* 20MHz */
...
static const struct mt76x0_radar_specs fcc_radar_specs[] = {
	/* 20MHz */
...
static const struct mt76x0_radar_specs jp_w56_radar_specs[] = {
	/* 20MHz */
...
static const struct mt76x0_radar_specs jp_w53_radar_specs[] = {
	/* 20MHz */
...
static void mt76x0_dfs_set_capture_mode_ctrl(struct mt76x0_dev *dev,
					     u8 enable)
{
	u32 data;
...
static bool mt76x0_dfs_check_chirp(struct mt76x0_dev *dev)
{
	bool ret = false;
	u32 current_ts, delta_ts;
	struct mt76x0_dfs_pattern_detector *dfs_pd = &dev->dfs_pd;
...
static void mt76x0_dfs_get_hw_pulse(struct mt76x0_dev *dev,
				    struct mt76x0_dfs_hw_pulse *pulse)
{
	u32 data;
...
static bool mt76x0_dfs_check_hw_pulse(struct mt76x0_dev *dev,
				      struct mt76x0_dfs_hw_pulse *pulse)
{
	bool ret = false;
	...
	case NL80211_DFS_FCC:
		...
		if (pulse->engine == 3) {
			ret = mt76x0_dfs_check_chirp(dev);
			break;
		}
...
static void mt76x0_dfs_tasklet(unsigned long arg)
{
	struct mt76x0_dev *dev = (struct mt76x0_dev *)arg;
	struct mt76x0_dfs_pattern_detector *dfs_pd = &dev->dfs_pd;
	u32 engine_mask;
	int i;
	...
	for (i = 0; i < MT_DFS_NUM_ENGINES; i++) {
		...
		pulse.engine = i;
		mt76x0_dfs_get_hw_pulse(dev, &pulse);

		if (!mt76x0_dfs_check_hw_pulse(dev, &pulse)) {
...
out:
	mt76x0_irq_enable(dev, MT_INT_GPTIMER);
}

static void mt76x0_dfs_set_bbp_params(struct mt76x0_dev *dev)
{
	u32 data;
	u8 i, shift;
	const struct mt76x0_radar_specs *radar_specs;
...
void mt76x0_dfs_adjust_agc(struct mt76x0_dev *dev)
{
...
void mt76x0_dfs_init_params(struct mt76x0_dev *dev)
{
	struct cfg80211_chan_def *chandef = &dev->mt76.chandef;

	if ((chandef->chan->flags & IEEE80211_CHAN_RADAR) &&
	    dev->dfs_pd.region != NL80211_DFS_UNSET) {
		mt76x0_dfs_set_bbp_params(dev);
		/* enable debug mode */
		mt76x0_dfs_set_capture_mode_ctrl(dev, true);
		...
		mt76x0_irq_disable(dev, MT_INT_GPTIMER);
		mt76_rmw_field(dev, MT_INT_TIMER_EN,
			       MT_INT_TIMER_EN_GP_TIMER_EN, 0);
	}
}

void mt76x0_dfs_init_detector(struct mt76x0_dev *dev)
{
	struct mt76x0_dfs_pattern_detector *dfs_pd = &dev->dfs_pd;

	dfs_pd->region = NL80211_DFS_UNSET;
	tasklet_init(&dfs_pd->dfs_tasklet, mt76x0_dfs_tasklet,
		     (unsigned long)dev);
}

void mt76x0_dfs_set_domain(struct mt76x0_dev *dev,
